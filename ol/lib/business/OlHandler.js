 ...
 * @LastEditTime: 2023-04-18 22:29:46
 */
...
import CustomMap from '../core/CustomMap'
import * as KLayerLoader from '../core/plugins/LayerHandler'

export default class OlHandler extends CustomMap {
    name = 'OlHandler class'
    // 地图容器
    mapContanier = ''
    constructor(mapContanier = 'ol-map-contanier') {
        super(mapContanier)
        // 图层集合
        ...
        // console.log('olUtils class' + ' init success~')
    }
    getMap() {
        return this.map
    }
    ...
    // localWmts(options) {
    //     console.log(options)
    //     let layer = KLayerLoader.getUserDefinedXYZ(options)
    ...
    interpolation(options) {
        let boxExtent = [117.57071648609964, 29.612017196294367, 118.56681691790034, 30.1558200567]
        let {
            map
        } = this
        ...
        const layer = KLayerLoader.getGeojsonLayerWithRender(options)
    ...
    drawKriging({
        ...
        if (values.length > 3) {
            ...
            let grid = kriging.grid(interpolationPolygons, letiogram, (ex[2] - ex[0]) / 200);
            //移除已有图层
            if (canvasLayer !== null) {
